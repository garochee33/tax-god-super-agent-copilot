...
curl -i http://localhost:5050/readiness
...
curl -X POST http://localhost:5050/api/v1/auth/dev-token
```
...
# List clients
curl http://localhost:5050/api/v1/clients -H "Authorization: $TOKEN"

# Create client
curl -X POST http://localhost:5050/api/v1/clients \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
  -d '{"name":"Acme Corp","email":"cfo@acme.com","filing_type":"business","status":"active"}'

# Update client
curl -X PATCH http://localhost:5050/api/v1/clients/<id> \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
  -d '{"notes":"Annual filing due March 15"}'

# Delete client
curl -X DELETE http://localhost:5050/api/v1/clients/<id> -H "Authorization: $TOKEN"
```
...
# Standard query
curl -X POST http://localhost:5050/api/v1/chat/query \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
...
# Citation search
curl -X POST http://localhost:5050/api/v1/chat/citations/search \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
...
```bash
curl "http://localhost:5050/api/v1/integrations/list" -H "Authorization: $TOKEN"
curl -X POST http://localhost:5050/api/v1/integrations/connect \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
  -d '{"provider":"google"}'
```
...
```bash
# ROI calculate
curl -X POST http://localhost:5050/api/v1/analytics/roi/calculate \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
  -d '{"investment_cost":60000,"incremental_revenue":240000}'

# ROI project
curl -X POST http://localhost:5050/api/v1/analytics/roi/project \
  -H "Authorization: $TOKEN" -H "Content-Type: application/json" \
...
# Usage analytics
curl http://localhost:5050/api/v1/analytics/usage -H "Authorization: $TOKEN"

# Circuit breaker status (admin only)
curl http://localhost:5050/api/v1/analytics/governance/circuit-breaker -H "Authorization: $TOKEN"
```
