...
class SecurityAgent(BaseAgent):
    ...
    def run(self, ctx: AgentContext) -> AgentResult:
        ...
        if not ctx.openai_api_key:
            summary = "SUMMARY: Offline security scan complete (no LLM)."
            evidence = run_offline_checks(
                ctx.repo_root,
                ctx.allowlist,
                [
                    rg_command("\"secret|token|password|credential|api key|auth|jwt\""),
                    rg_command("\"encrypt|crypto|hash|bcrypt|scrypt|pbkdf\""),
                ],
            )
