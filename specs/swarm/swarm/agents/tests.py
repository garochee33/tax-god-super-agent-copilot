...
from swarm.tools.offline import run_offline_checks, rg_command
...
class TestsAgent(BaseAgent):
    ...
    def run(self, ctx: AgentContext) -> AgentResult:
        ...
        if not ctx.openai_api_key:
            summary = "SUMMARY: Offline test scan complete (no LLM)."
            evidence = run_offline_checks(
                ctx.repo_root,
                ctx.allowlist,
                [
                    rg_command("\"pytest|unittest|test_\""),
                    rg_command("\"TODO|FIXME\"", "tests"),
                ],
            )
