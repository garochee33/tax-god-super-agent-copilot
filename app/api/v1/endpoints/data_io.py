...
async def export_quickbooks(db: DBSession, current_user: CurrentUser):
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=taxgod_export.iif"},
    )
...
async def export_all_zip(db: DBSession, current_user: CurrentUser):
    ...
    return Response(
        ...
        headers={"Content-Disposition": "attachment; filename=taxgod_export.zip"},
    )
...
async def export_entity_csv(entity: str, db: DBSession, current_user: CurrentUser):
    ...
    if entity not in VALID_ENTITIES:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid entity. Must be one of: {VALID_ENTITIES}"
        )
    csv_data = await export_to_csv(db, current_user.id, entity)
    return Response(
        content=csv_data, media_type="text/csv", headers={"Content-Disposition": f"attachment; filename={entity}.csv"}
    )
